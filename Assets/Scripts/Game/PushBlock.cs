...
public class PushBlock : MonoBehaviour, ISavable
{
	...
	bool CheckPath()
	{
		...
		RaycastHit2D[] hits = Physics2D.BoxCastAll ((Vector2)transform.position + _col.offset + (new Vector2(moveDir.x /2, moveDir.y /2)), _col.size, 0, moveDir, 0.1f, 1 << LayerMask.NameToLayer ("PushBlockArea") | 1 << LayerMask.NameToLayer ("Wall"));
